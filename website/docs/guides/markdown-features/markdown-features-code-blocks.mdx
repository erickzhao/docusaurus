...
By default, the Prism [syntax highlighting theme](https://github.com/FormidableLabs/prism-react-renderer#theming) we use is [Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/src/themes/palenight.js). You can use choose to use different themes using the `themeConfig.prism` field in your docusaurus.config.js.

For example, if you want to use the Night Owl theme, which has both dark and light versions available:
